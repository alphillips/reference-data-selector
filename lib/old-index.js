...
var RefDataSelector = function (_React$Component) {
  ...
  function RefDataSelector(props) {
    ...
    _this.onChange = function (val, src) {
      ...
      if (code) {
        if (_this.props.onChange) {
          _this.props.onChange(code.value, src);
        }
      } else {
    ...
    _this.host = process.env.REACT_APP_API_HOST || '';
    if (props.url) {
      _this.urlPrefix = window.location.protocol + '//' + window.location.hostname + (window.location.port ? ':' + window.location.port : '') + props.url;
    } else {
  ...
  RefDataSelector.prototype.render = function render() {
    ...
    return _react2.default.createElement(
      'div',
      { className: "text-group ref-data-selector " + errorClass, style: containerStyle },
